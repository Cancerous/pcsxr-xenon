...
unsigned char ubGloAlpha; // texture alpha
unsigned char ubGloColAlpha; // color alpha
...
struct XenosSurface * gTexName; // binded texture
...
__inline void PRIMdrawTexturedQuad(OGLVertex* vertex1, OGLVertex* vertex2,
    ...
    PsxVertex[1].u = vertex2->sow;
    PsxVertex[1].v = vertex2->tow;
    PsxVertex[1].x = vertex2->x;
    PsxVertex[1].y = vertex2->y;
    PsxVertex[1].z = vertex2->z;

    PsxVertex[2].u = vertex3->sow;
    PsxVertex[2].v = vertex3->tow;
    PsxVertex[2].x = vertex3->x;
    PsxVertex[2].y = vertex3->y;
    PsxVertex[2].z = vertex3->z;

    PsxVertex[3].u = vertex4->sow;
    PsxVertex[3].v = vertex4->tow;
    PsxVertex[3].x = vertex4->x;
...
__inline void PRIMdrawTri2(OGLVertex* vertex1, OGLVertex* vertex2,
    ...
    PsxVertex[3].x = vertex3->x;
    PsxVertex[3].y = vertex3->y;
    PsxVertex[3].z = vertex3->z;

    PsxVertex[2].x = vertex4->x;
...
static int obm1 = XE_BLEND_ZERO;
static int obm2 = XE_BLEND_ZERO;
...
SemiTransParams MultiColTransSets[4] = {
    ...
    {XE_BLEND_ONE, XE_BLEND_ONE, 255},
    {XE_BLEND_ONE, XE_BLEND_INVSRCCOLOR, 255},
    {XE_BLEND_SRCALPHA, XE_BLEND_ONE, 127}
};
...
void SetRenderMode(uint32_t DrawAttributes, BOOL bSCol) {
    ...
    {
        struct XenosSurface * currTex = NULL;
        if (bUsingTWin) {
...
void primBlkFill(unsigned char * baseAddr) {
    ...
    if (iOffscreenDrawing) {
        ...
        FillSoftwareArea(sprtX, sprtY, sprtW, sprtH, BGR24to16(color));
    }
}
...
void primMoveImage(unsigned char * baseAddr) {
    ...
    if (iGPUHeight == 1024 && imageSY > 1024) return;
...
void primTileS(unsigned char * baseAddr) {
    ...
    if (iOffscreenDrawing) {
        ...
        offsetPSX4();
        if (bDrawOffscreen4()) {
            if (!(iTileCheat && sprtH == 32 && color == 0x60ffffff)) // special cheat for certain ZiNc games
            {
...
void primSprt8(unsigned char * baseAddr) {
    ...
    if (ubOpaqueDraw) {
        SetZMask4O();
        if (bUseMultiPass) SetOpaqueColor(color);
        DEFOPAQUEON
...
void primPolyG4(unsigned char * baseAddr) {
    ...
    uint32_t color_2 = GETLE32(&gpuData[4]);
    uint32_t color_3 = GETLE32(&gpuData[6]);
    ...
    vertex[0].c.lcol = color_0;
    vertex[1].c.lcol = color_1;
    vertex[2].c.lcol = color_2;
    vertex[3].c.lcol = color_3;
...
void primPolyG3(unsigned char *baseAddr) {
    ...
    if (iOffscreenDrawing) {
        ...
        if (bDrawOffscreen3()) {
            InvalidateTextureAreaEx();
            drawPoly3G(color_0, color_1, color_2);
        }
    }
...
void primPolyGT4(unsigned char *baseAddr) {
    ...
    uint32_t color_0 = GETLE32(&gpuData[0]);
    uint32_t color_1 = GETLE32(&gpuData[3]);
    uint32_t color_2 = GETLE32(&gpuData[6]);
    uint32_t color_3 = GETLE32(&gpuData[9]);
    ...
    if (!bUseMultiPass && !bGLBlend) {
        vertex[0].c.lcol = DoubleBGR2RGB(color_0);
        vertex[1].c.lcol = DoubleBGR2RGB(color_1);
        vertex[2].c.lcol = DoubleBGR2RGB(color_2);
        vertex[3].c.lcol = DoubleBGR2RGB(color_3);
    } else {
        vertex[0].c.lcol = color_0;
        vertex[1].c.lcol = color_1;
        vertex[2].c.lcol = color_2;
...
void primLineGEx(unsigned char *baseAddr) {
    ...
    vertex[0].c.lcol = vertex[3].c.lcol = color;
...
void primLineG2(unsigned char *baseAddr) {
    ...
    if (iOffscreenDrawing) {
        ...
        if (bDrawOffscreen4()) {
            InvalidateTextureAreaEx();
            drawPoly4G(color_0, color_1, color_0, color_1);
        }
    }
