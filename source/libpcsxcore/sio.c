...
//					SysPrintf("%x: %x, %x, %x, %x\n", CtrlReg&0x2002, buf[2], buf[3], buf[4], buf[5]);
        ...
        case 2:
            ...
            if (!Config.UseNet) {
                switch (CtrlReg & 0x2002) {
                    case 0x0002: buf[parp] = PAD1_poll(value);
                        break;
                    case 0x2002: buf[parp] = PAD2_poll(value);
                        break;
                }
            }
    ...
    switch (value) {
        case 0x01: // start pad
            ...
            if (!Config.UseNet) {
                switch (CtrlReg & 0x2002) {
                    case 0x0002: buf[0] = PAD1_startPoll(1);
                        break;
                    case 0x2002: buf[0] = PAD2_startPoll(2);
                        break;
                }
            } else {
...
void LoadMcd(int mcd, char *str) {
    ...
    if (*str == 0) {
        sprintf(str, "uda:/memcards/card%d.mcd", mcd);
